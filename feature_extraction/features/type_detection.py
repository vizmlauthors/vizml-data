...
def detect_field_type(v, num_samples=1000):
	...
	for e in v:
		if e is None or e == '':
			continue
		e = str(e)
		if detect_string(e): type_scores['string'] += type_weights['string']
		if detect_integer(e): type_scores['integer'] += type_weights['integer']
		if detect_decimal(e): type_scores['decimal'] += type_weights['decimal']
		if detect_time(e): type_scores['time'] += type_weights['time']    
	score_tuples = []
