...
def extract_chunk_features(args):
	...
	for chart_num, chart_obj in df.iterrows():
		...
		if absolute_chart_num % 100 == 0:
			print('[Batch %s / Chunk %s][%s] %.1f: %s %s' % (batch_num, chunk_num, absolute_chart_num, time() - start_time, fid, 'https://plot.ly/~{0}/{1}'.format(*fid.split(':'))))
		...
		try:
			extraction_results = extract_features_from_fields(sorted_fields, compute_features_config, chart_obj=chart_obj, fid=fid)
...
def write_batch_results(batch_results, features_dir_name, write_header=False):
	...
	for r in batch_results:
		...
		for i, dataset_features in enumerate(r['dataset_features_by_num_fields']):
	...
	persist_features(concatenated_results, MAX_FIELDS, features_dir_name, feature_names_by_type, write_header=write_header)
...
if __name__ == '__main__':
	...
	features_dir_name = 'features_{}'.format(current_dt)
	...
	if parallelize:
		...
		for i, chunk in enumerate(raw_df_chunks):
			...
			if (chunk_num == (batch_size * batch_num)):
				batch_results = process_batch(chunk_batch, batch_num, batch_size, pool, chunk_size=CHUNK_SIZE)
				write_batch_results(batch_results, features_dir_name, write_header=first_batch)	
				batch_num += 1
				chunk_batch = []	
				first_batch=False									
			pool.close()
		...
		# Process left overs
		pool = Pool(n_jobs)		
		remaining_batch_results = process_batch(chunk_batch, batch_num, batch_size, pool, chunk_size=CHUNK_SIZE)
		write_batch_results(remaining_batch_results, features_dir_name, write_header=first_batch)					
		pool.close()
