...
def check_if_line_chart(d):
	if d.get('mode') in ['lines+markers', 'lines']: return True
	if d.get('line') and len(d.get('line').keys()) > 0: return True
	if d.get('marker') and 'line' in d.get('marker') and d.get('marker').get('line') and d.get('marker').get('line').get('color') != 'transparent': return True
	return False
...
def extract_field_outcomes(chart_obj):
	...
	for field_name, d in fields.items():
		...
		field_outcomes[field_id] = OrderedDict([ (f, None) for f in field_outcome_feature_names ])
	...
	try:
		...
		for d in chart_data:
			t = d.get('type')
			if t:
				if t == 'scatter' and check_if_line_chart(d): t = 'line'
				num_traces += 1
		...
		# print('x_layout_axes', x_layout_axes)
		# print('x_axes', x_axes)
	except Exception as e:
		# print('Field encoding extraction error:', e)
